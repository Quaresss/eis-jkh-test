export const mapMeterType = (typeList: string[]) => {
...
export const buildAddress = (houseAddress?: string, apartment?: string) => {
  ...
  const address = [houseAddress?.trim(), apartmentPart].filter(Boolean).join(', ');
  return address || '—';
};

export const buildPaginationItems = (current: number, maxPage: number): Array<number | '...'> => {
  if (maxPage <= 7) return Array.from({ length: maxPage }, (_, index) => index + 1);
  if (current <= 4) return [1, 2, 3, 4, 5, '...', maxPage];
  if (current >= maxPage - 3) return [1, '...', maxPage - 4, maxPage - 3, maxPage - 2, maxPage - 1, maxPage];
